...
// ДОМАШКА СЛОЖНАЯ !!!!!!!!!!!!!!!!!!!!!!!!!!!!! почему не выводит массив!!!!
...
// const getAllSkills = (arr) => arr.reduce((acc, val) => acc.concat(val['skills']), [])
// .map(x=>!uniq.includes(x)? uniq.push(x): continue);
// // 

// const getAllSkills = (arr) =>  { 
//   arr.reduce((acc, val) => acc.concat(val['skills']), []).map(x=>{
//   if(!uniq.includes(x)){
//     uniq.push(x);
//   }})
//   return uniq;
...
*/
// const getPropValues = (arr, prop) => arr.map(x => (x[prop]));
...
//   { name: 'Chelsey', inactiveDays: 85, isActive: true }
// ];
// const setGuestState = (users, period) => users.map(x=>x.inactiveDays<period?x.isActive = true: x.isActive =false) 
// // Вызовы функции для проверки
// console.log(
//   setGuestState(guests, 10)
// ); // Объекты Mango, Ajax, Chelsey получат isActive false, а Poly наоборот true
...
/*
  ...
  PS: обязательно используй перебирающие методы массивов, никаких for!
*/
// const getActiveGuests=(name)=>name.filter(x => x.isActive === true);
// const guests = [
//   { name: "Mango", age: 20, isActive: true },
//   { name: "Poly", age: 18, isActive: false },
//   { name: "Ajax", age: 30, isActive: true },
...
//   { name: "Chelsey", age: 45, isActive: false }
// ];
// const getGuestsOlderThan = (user, num)=> user.filter(name => name.age>num);
// // Вызовы функции для проверки
// console.log(getGuestsOlderThan(guests, 25)); // массив из 2-х объектов Ajax и Chelsey
...
//   { id: 4, name: 'Chelsey', age: 45 }
// ];
// const getGuestById = (arr, num) => arr.find(x => x.id === num)
// // Вызовы функции для проверки
// console.log(
//   getGuestById(guests, 3)
// ); // {id: 3, name: 'Ajax', age: 30}
...
// */
//  let getTotal=(order) =>Object.values(order).reduce((acc, val)=> acc + val, 0)

// // /*let getTotal=(order) =>order.reduce((acc, val)=> acc + order[val], 0)*/почему не работает????
...
*/
// const isGuestsActive = (guests)=> guests.every(x=>x.active);
// // Вызовы функции для проверки
// console.log(
//   isGuestsActive([
